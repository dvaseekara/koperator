...
func (r *Reconciler) getConfigString(bConfig *v1beta1.BrokerConfig, id int32, serverPass, clientPass string, superUsers []string, log logr.Logger) string {
	...
	if err := t.Execute(&out, map[string]interface{}{
		...
		"CruiseControlBootstrapServers":      getInternalListener(brokerListenersConfig.InternalListeners, id, r.KafkaCluster.Namespace, r.KafkaCluster.Name, r.KafkaCluster.Spec.HeadlessServiceEnabled),
		"StorageConfig":                      generateStorageConfig(bConfig.StorageConfigs),
		"AdvertisedListenersConfig":          generateAdvertisedListenerConfig(id, brokerListenersConfig, r.KafkaCluster.Namespace, r.KafkaCluster.Name, r.KafkaCluster.Spec.HeadlessServiceEnabled),
...
func generateAdvertisedListenerConfig(id int32, l *v1beta1.ListenersConfig, namespace, crName string, headlessServiceEnabled bool) string {
	advertisedListenerConfig := []string{}
	for _, eListener := range l.ExternalListeners {
		advertisedListenerConfig = append(advertisedListenerConfig,
			fmt.Sprintf("%s://%s:%d", strings.ToUpper(eListener.Name), eListener.Hostname, eListener.ExternalStartingPort+id))
	}
