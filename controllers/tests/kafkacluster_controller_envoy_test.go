...
func expectEnvoyWithConfigAz1Tls(kafkaCluster *v1beta1.KafkaCluster) {
	...
	Expect(loadBalancer.Spec.Ports[1].Protocol).To(Equal(corev1.ProtocolTCP))
	Expect(loadBalancer.Spec.Ports[1].Port).To(BeEquivalentTo(8080))
	Expect(loadBalancer.Spec.Ports[1].TargetPort.IntVal).To(BeEquivalentTo(8080))

	Expect(loadBalancer.Spec.Ports[2].Name).To(Equal("tcp-admin"))
	Expect(loadBalancer.Spec.Ports[2].Protocol).To(Equal(corev1.ProtocolTCP))
	Expect(loadBalancer.Spec.Ports[2].Port).To(BeEquivalentTo(8081))
	Expect(loadBalancer.Spec.Ports[2].TargetPort.IntVal).To(BeEquivalentTo(8081))
